...
/**
 * `expo-media-library` has no web implementation: the module throws the
 * instant it is imported there, before `saveDayCard` is ever called. Required
 * lazily, and only on native, so the web build never evaluates it at all.
 */
 ...
 * either order. Returns false if it is refused, or on web, where there is no
 * camera roll to write to.
 */
