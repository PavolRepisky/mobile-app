...
export interface ProfileLayoutProps {
  ...
  /**
   * Lets `children` stretch to fill whatever the scroll's own viewport leaves
   * over, rather than sitting at its own content height. The To-do home wants
   * its grid to stand in for the page rather than sit as one block on it; a
   * screen whose content is its own reason to scroll leaves this off.
   */
  fill?: boolean;
}
  ...
  return (
    ...
    <View style={styles.root}>
      <ScreenScroll
        ...
        contentContainerStyle={fill ? styles.fillContent : undefined}
      >
        ...
        {fill ? <View style={styles.fillChildren}>{children}</View> : children}
      </ScreenScroll>
...
const styles = StyleSheet.create({
  ...
  // A ScrollView's content container only grows to fit the screen when
  // something inside it is left to claim the space: this stretches the box
  // that gets to, so `children` fills whatever the scroll left over.
  fillContent: {
    flexGrow: 1,
  },
  fillChildren: {
