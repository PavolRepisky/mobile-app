...
**`FriendCard`**, **`ReviewCard`** take their domain object
...
`'discover'|'friends'|'todo'|'calendar'|'profile'` (outline until focused), a
...
`content.ts`); formatting helpers in `lib/format.ts`.
